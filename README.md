...
## Deploy the public reference runtime

[![Deploy to Render](https://render.com/images/deploy-to-render-button.svg)](https://render.com/deploy?repo=https://github.com/berknerkus-commits/berk-ai-operations-control-plane)

The Blueprint provisions only free Render resources: one FastAPI web service and one Free Render Postgres database. Render Free Postgres expires after 30 days and Free web services sleep when idle; this is a review deployment, not a production availability claim. Secrets are generated by Render and are never committed.

n8n is intentionally CI-only: a clean GitHub Actions job starts the official n8n container, imports `n8n/ci-proof.json`, invokes its webhook, and verifies the response includes a real n8n execution ID and a FastAPI health response. This avoids consuming a second always-on Free web-service budget while keeping the n8n runtime evidence reproducible.
